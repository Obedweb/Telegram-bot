...
async function sendAsFile(Hunter, chatId, msgId, content, prefix) {
    const detected = detectLang(content);
    const filePath = path.resolve(tempDir, `output_${Date.now()}${detected.ext}`);
    try {
        fs.writeFileSync(filePath, content);
        const stream = fs.createReadStream(filePath);
        await Hunter.sendDocument(chatId, stream, {
...
const HUNTER_YTS_API = 'https://yts.huntertech.co.ke';

async function searchHunterTechYts(query) {
    try {
        const response = await axios.get(`${HUNTER_YTS_API}/`, {
            params: { q: query },
            timeout: 10000
        });
        ...
        console.error('HunterTechYts error:', error.message);
        return [];
    }
}
...
async function uploadToGithubCdn(buffer, filename) {
    ...
    const { data } = await axios.post('https://ghbcdn.huntertech.co.ke/api/upload.php', form, {
        headers: form.getHeaders(),
        maxContentLength: Infinity,
        maxBodyLength: Infinity
    });
...
async function uploadToHunterCdn(buffer, filename, deleteKey = '') {
    const form = new FormData();
    form.append('file', buffer, {
        filename: filename,
        contentType: getFileContentType(path.extname(filename)),
    ...
    const { data } = await axios.post('https://cdn.huntertech.co.ke/api/upload.php', form, {
        headers: form.getHeaders(),
        maxContentLength: Infinity,
        maxBodyLength: Infinity
    });
