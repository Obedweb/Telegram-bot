...
const { formatCodeHtml, sendAsFile, execAsync, fs, path, util } = require('../hunt/gmdHelpers');
...
async (msg, Hunter, conText) => {
    ...
    try {
        ...
        if (output.length > 4000) {
            await sendAsFile(Hunter, conText.chatId, msg.message_id, output, 'eval');
        } else {
            await Hunter.sendMessage(conText.chatId, formatCodeHtml(output), {
        ...
        if (errMsg.length > 4000) {
            await sendAsFile(Hunter, conText.chatId, msg.message_id, errMsg, 'eval_error');
        } else {
...
async (msg, Hunter, conText) => {
    ...
    try {
        ...
        if (output.length > 4000) {
            await sendAsFile(Hunter, conText.chatId, msg.message_id, output, 'shell');
        } else {
            await Hunter.sendMessage(conText.chatId, formatCodeHtml(output), {
                reply_to_message_id: msg.message_id,
                parse_mode: 'HTML'
        ...
        if (error.stdout && error.stdout.length > 0) {
            ...
            if (combined.length > 4000) {
                await sendAsFile(Hunter, conText.chatId, msg.message_id, combined, 'shell_error');
            } else {
